...
The API spec is annotated in [app/main.py](https://github.com/jakehadar/bikeshare-json-api/blob/master/app/main.py)
...
Usage
-----
Once bikeshare-json-api is installed, start the FastAPI app with `uvicorn` from the command line.

``` {.sourceCode .bash}
 $ uvicorn app.main:app
```

Alternatively, use the `fastapi` CLI (installed via the `fastapi[standard]` extra) for local development, or to deploy to [FastAPI Cloud](https://fastapicloud.com):

``` {.sourceCode .bash}
 $ fastapi dev app/main.py
 $ fastapi deploy app/main.py
```

Open a browser or `curl` to `http://127.0.0.1:8000/app/api/v1.0/systems` (sub in your own local host & port) and start exploring!

Interactive API docs (Swagger UI) are available at `http://127.0.0.1:8000/docs`, and the raw OpenAPI schema at `http://127.0.0.1:8000/openapi.json`.

Note: If the API returns a 404 "Not Found" response, check that your request URL has a valid endpoint path, like `/app/api/v1.0/systems` (See _Routes_ table below, or `/docs`). For example, `GET http://127.0.0.1:8000/` would return a 404 because `/` is not a route in app/main.py.
...
---------------------------|--------|-------------------------------------------------------------------------------
system_detail              |GET     |/app/api/v1.0/system/\<system_id\>
system_feed_detail         |GET     |/app/api/v1.0/system/\<system_id\>/feed/\<feed_name\>
system_feeds               |GET     |/app/api/v1.0/system/\<system_id\>/feeds
system_station_detail      |GET     |/app/api/v1.0/system/\<system_id\>/station/\<station_id\>
system_station_information |GET     |/app/api/v1.0/system/\<system_id\>/station/\<station_id\>/status
system_station_status      |GET     |/app/api/v1.0/system/\<system_id\>/station/\<station_id\>/information
system_stations            |GET     |/app/api/v1.0/system/\<system_id\>/stations
